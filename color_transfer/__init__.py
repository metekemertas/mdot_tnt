...
def transfer_colors(
    ...
    *,
    ...
    if gamma_f > 2 ** 10 and dtype != th.float64:
        warnings.warn(
            f"Switching to float64 for gamma_f > 2^10 (output will be {dtype})."
        )
    ...
    from_cs = _lab_to_srgb if color_space == "lab" else (lambda x: x / x.amax(dim=-1, keepdim=True).clamp(min=1.0))
